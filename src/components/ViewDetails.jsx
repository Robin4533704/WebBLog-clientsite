...
import { FaFacebookF, FaTwitter, FaTelegramPlane, FaLink, FaHeart, FaStar } from "react-icons/fa";
    ...
    const fetchBlog = async () => {
      try {
        ...
        if (blogData) {
          ...
          if (auth.currentUser && blogData.likedBy?.includes(auth.currentUser.uid)) setLiked(true);

          const relatedRes = await axiosIntals(`/blogs?category=${encodeURIComponent(blogData.category)}`);
  ...
  return (
    ...
    >
      ...
      <motion.div className="flex flex-wrap gap-2 mb-6" variants={fadeUp}>
        {blog?.tags?.map((tag, i) => (
          <Link key={i} to={`/blogs?tag=${tag}`} className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm hover:bg-blue-200 transition">
            #{tag}
          </Link>
        ))}
      </motion.div>
      ...
      <motion.div className="flex flex-wrap items-center gap-3 mb-6" variants={fadeUp}>
        ...
        <motion.button onClick={() => handleSocialShare("facebook")} className="bg-blue-600 text-white px-3 py-2 rounded hover:bg-blue-700"><FaFacebookF /></motion.button>
        <motion.button onClick={() => handleSocialShare("twitter")} className="bg-sky-400 text-white px-3 py-2 rounded hover:bg-sky-500"><FaTwitter /></motion.button>
        <motion.button onClick={() => handleSocialShare("telegram")} className="bg-blue-400 text-white px-3 py-2 rounded hover:bg-blue-500"><FaTelegramPlane /></motion.button>
        <motion.button onClick={() => handleSocialShare("copy")} className="bg-gray-400 text-white px-3 py-2 rounded hover:bg-gray-500"><FaLink /></motion.button>

        <Link to="/blogs" className="bg-amber-500 text-white px-4 py-2 rounded hover:bg-amber-600">Back to Blogs</Link>
      </motion.div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex text-yellow-400">
                    {[...Array(rev.rating || 5)].map((_, i) => <FaStar key={i} />)}
                  </div>
                </div>
        ...
        {user && (
          <ReviewForm
            ...
            onNewReview={(newReview) =>
              setBlog((prev) => ({ ...prev, reviews: [...prev.reviews, newReview] }))
            }
          />
        )}
      </motion.div>
      ...
      {relatedBlogs.length > 0 && (
        <motion.div className="mt-12" variants={fadeUp}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {relatedBlogs.map((rb) => (
              <Link key={rb._id} to={`/blogs/${rb._id}`} className="border rounded-lg overflow-hidden shadow hover:shadow-lg transition transform hover:scale-105">
